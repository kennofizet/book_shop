...
<div id="content" class="fl-right">
    <div class="section" id="title-page">
        <div class="clearfix">
            <h3 id="index" class="fl-left">Sửa Danh mục sản phẩm > {{$category_detail->title}}</h3>
        </div>
    </div>
    <div class="section" id="detail-page">
        <div class="section-detail">
            <form method="POST" id="form-edit-product-category" action="{{ route('source.api.admin.product.category.create') }}">
                {{csrf_field()}}
                <label for="title">Tên</label>
                <input type="text" name="title" value="{{$category_detail->title}}" id="title" style="width: 100%">
                ...
                <div class="col-md-12" style="margin-top: 20px">
                                ...
                                <select name="parent">
                                    @if($list_category->count() > 0)
                                        ...
                                        @foreach($list_category as $detail_category)
                                          @if($category_detail->parent == $detail_category->id)
                                            <option value="{{$detail_category->id}}" selected>{{$detail_category->title}}</option>
                                          @else
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" >
                            <option value="">-- Chọn trạng thái --</option>
                            <option value="1" @if($category_detail->status == 1) selected @endif>Hiện</option>
                            <option value="0" @if($category_detail->status == 0) selected @endif>Ẩn</option>
                        </select>
                    </div>
                </div>
                <p style="float: right;" class="btn btn-success" type="submit" name="btn-submit" id="btn-submit">Lưu</p>
                 <input type="text" name="id" value="{{$category_detail->id}}" style="display: none;">
            </form>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    jQuery(function($) {
      $('#form-edit-product-category').on('submit',function(e){
          e.preventDefault();
          $.ajax({
            url:"{{ route('source.api.admin.product.category.edit') }}",
            method:"POST",
            ...
            error: function(jqXhr, json, errorThrown){
                ...
                if (jqXhr['responseJSON']['errors']['id']) {
                    var data_mess = "Danh mục sửa không tồn tại !";
                    pushNotify("error",1,data_mess);
                };
            }
          });
      });
...
<script type="text/javascript">
    $('#btn-submit').on('click',function () {
        $('#form-edit-product-category').submit();
    });
</script>
@endsection
