...
<div id="content" class="fl-right">
    ...
    <div class="section" id="detail-page">
        <div class="section-detail">
            <form method="POST" id="form-add-new-product" action="" enctype="multipart/form-data">
                {{csrf_field()}}
                ...
                <div style="margin-top: 10px">
                    ...
                    <textarea id="desc-detail-description-product" class="ckeditor"></textarea>
                </div>
                <label for="content">Chi tiết</label>
                <textarea id="desc-detail-content-product" class="ckeditor"></textarea>
                            ...
                            @foreach($list_category as $detail_category)
                                ...
                                @if($detail_category->Child->first())
                                    @foreach($detail_category->Child as $category_child_1)
                                        <option value="{{$category_child_1->id}}">- {{$category_child_1->title}}</option>
                                        @if($category_child_1->Child->first())
                                            @foreach($category_child_1->Child as $category_child_2)
                                                <option value="{{$category_child_2->id}}">- - {{$category_child_2->title}}</option>
                                                @if($category_child_2->Child->first())
                                                    @foreach($category_child_2->Child as $category_child_3)
                                                        <option value="{{$category_child_3->id}}">- - - {{$category_child_3->title}}</option>
                                                        @if($category_child_3->Child->first())
                                                            @foreach($category_child_3->Child as $category_child_4)
                                                                <option value="{{$category_child_4->id}}">- - - - {{$category_child_4->title}}</option>
                                                            @endforeach
                                                        @endif
                                                    @endforeach
                                                @endif
...
<script type="text/javascript">
    jQuery(function($) {
      $('#form-add-new-product').on('submit',function(e){
          e.preventDefault();
          $.ajax({
            url:"{{ route('source.api.admin.product.create') }}",
            method:"POST",
